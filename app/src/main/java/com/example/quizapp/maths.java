 ...
 */
public class maths extends SQLiteAssetHelper {
   ...
   //     Cursor c = sqlite.rawQuery("SELECT " + Question + " FROM " + Table_name + " WHERE " + uid + " = " + i + "", null);//cursor to that query
       // Cursor c = sqlite.rawQuery("SELECT " + Question + "  WHERE " + uid + " = " + i + "", null);//cursor to that query


        Cursor c=null;
        String SQLQuery=String.format("SELECT * FROM Question WHERE _id="+i+"");
